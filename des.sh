...
echo "Text: $plain_text"
echo "Key: $key"
plain_text=$(echo $plain_text | rev)
key=$(echo $key | rev)
...
do
    paritykey+="0$(parity $(echo "$ascii_key" | awk '{print $"'$v'"}'))"
done
echo "Paritified key: $paritykey"

CDs $paritykey
Ks
cipher $ascii_plain_text $decryptOrNot
